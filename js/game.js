...
game.States.preload = function(){
	this.preload = function(){
		...
		game.load.image('flag','assets/flag.png'); //背景
		
    	game.load.image('ground','assets/ground.png'); //地面
    	game.load.image('title','assets/title.png'); //游戏标题
    	game.load.spritesheet('bird','assets/bird.png',50,50,3); //鸟
...
game.States.menu = function(){
	this.create = function(){
		musica = game.add.audio('theme');
		musica.loop = true;
		musica.volume = 1.5;
		musica.allowMultiple = false;
		musica.play();
		game.add.tileSprite(0,0,game.width,game.height,'background').autoScroll(-10,0); //背景图
		game.add.tileSprite(0,0,game.width,game.height,'flag').autoScroll(-20,0); //背景图
		...
		bird.scale.setTo(1.2, 1.2);
		bird.anchor.setTo(0,0.3);
...
game.States.play = function(){
	this.create = function(){
		...
		this.flag = game.add.tileSprite(0,0,game.width,game.height,'flag');//背景图
	...
	this.statrGame = function(){
		...
		this.flag.autoScroll(-(this.gameSpeed/8),0);
	...
	this.fly = function(){
		...
		this.soundFly.volume = 0.7;
		this.soundFly.play();
	}

	this.hitPipe = function(){
		if(this.gameIsOver) return;
		this.soundHitPipe.volume = 0.6;
	...
	this.hitGround = function(){
		...
		this.soundHitGround.volume = 0.7;
